...
import { createOrder, singleUserByIDs } from "../../../services";
...
const Checkout = ({ setCheckout }) => {
    ...
    const cbid = JSON.parse(localStorage.getItem("cbid"));

    useEffect(() => {
        async function getUser() {
            const response = await singleUserByIDs(cbid)
    ...
    async function handleOrderSubmit(event) {
        ...
        try {
            const orderDetails = {
                ...
                orders_id: generateRandomString(10),
                user: {
                    name: user.name,
                    email: user.email,
                    id: user.id
            ...
            clearCart();
            navigate("/order-summary", { state: { data: response.data.result, status: true } });
        } catch (error) {
